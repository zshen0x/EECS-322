...
let calc_gen_and_kill insts_arr =
  ...
  let gen_and_kill_each_inst = function
    ...
    | Expr [Atom "call"; Atom u; Atom nat] ->
      let gen = args @(foo u) and kill = caller_save @result in
      (SS.of_list gen, SS.of_list kill)
    | Expr [Atom "tail-call"; Atom u; Atom nat] ->
      let gen = args @(foo u) and kill = [] in
      (SS.of_list gen, SS.of_list kill)
    | Expr [Atom "return"] ->
  ...
  let rec fix_point () =
    ...
    let iteration () =
      ...
      for i=s-1 downto 0 do
        ...
        if not (SS.equal in_arr.(i) in_prime) || not (SS.equal out_arr.(i) out_prime)
...
let () =
  match Sys.argv with
  | [|_; filename|] ->
    List.iter (fun f -> print_two_sests (liveness_analysis f) "in" "out") (parse_file filename)
  | _ -> run_test ()
