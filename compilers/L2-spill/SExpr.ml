(* modified version of simple s-expr paser lib *)
type atom = Atom_unit
          | Atom_int of int
          | Atom_sym of string

type sexpr = Atom of string
           | Expr of sexpr list
...
type state =
  ...
  | Parse_comment of state        (* comment should be ignored *)
    ...
    | Some c ->
        ...
        | ' ' | '\n' | '\r' | '\t' ->
            ...
            | Parse_string(_, s, sl) ->
                Buffer.add_char s c;
                aux(Parse_string(false, s, sl))
            | Parse_comment subst when c = '\n' ->
                aux subst (* only \n are considered new line thus end of comment *)
            | Parse_comment _ -> aux st
            end
        | '"' ->
            (* '"' *)
        ...
        | ';' ->
            ...
            | Parse_content _
            | Parse_word _
            | Parse_string _ -> aux(Parse_comment st)
            | Parse_comment _ -> aux st    (* never nest Parse_commnet *)
            end
        | _ ->
            begin match st with
            | Parse_root _ ->
