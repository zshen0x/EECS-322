...
(* d -> sexpr list *)
let rec compile_l3_d my_compile_l3_v dst inst_d =
  let ending =
    if dst = result_reg then [Expr [Atom "return"]] else [] in
  ...
  let function_call_insts f_sexpr vars_sexprs =
    ...
    if dst = result_reg && vars_len <= args_len then begin
      (* tail call *)
      vars2args_insts @ [call_inst (Atom "tail-call")]
    end
    else begin
      (* normal calls *)
  ...
  in
  let runtime_call_insts ?(extras=[]) f_sexpr vars_sexpr =
  ...
  begin match inst_d with
    ...
    | L3NewArray (size_v, init_v) ->
      ...
      runtime_call_insts (Atom "allocate") [size_sexpr; init_sexpr]
    | L3NewTuple vals_v ->
      let loc = ref 0 in
      let val_assign arr val_sexpr =
        let n8 = Atom (string_of_int (8 * (!loc + 1))) in
        begin
          incr loc;
          Expr [Expr [Atom "mem"; arr; n8]; Atom "<-"; val_sexpr]
        end
      in
      ...
      let assign_initals_insts = List.map (val_assign result_reg) initials_sexprs in
      runtime_call_insts (Atom "allocate") [size_sexpr; init_sexpr] ~extras: assign_initals_insts
    | L3Aref (arr_v, pos_v) ->
    ...
    | L3Print val_v ->
      ...
      runtime_call_insts (Atom "print") arg_sexprs
    | L3V v_v ->
      let var_expr = my_compile_l3_v (encode_l3_v v_v) in
      Expr [dst; Atom "<-"; var_expr] :: ending
    | L3MakeClosure (l, v_v) ->
      compile_l3_d my_compile_l3_v dst (L3NewTuple [L3Label l; v_v])
    | L3ClosureProc arr_v ->
      compile_l3_d my_compile_l3_v dst (L3Aref (arr_v, L3Num (Int64.zero)))
    | L3ClosureVars arr_v ->
      compile_l3_d my_compile_l3_v dst (L3Aref (arr_v, L3Num (Int64.one)))
    | L3Read -> runtime_call_insts (Atom "read") []
  end

(* important now evaluation order matters since passing mutable hashtable around *)
(* e -> expr list *)
let rec compile_l3_e l3_label_prefixer l3_var_map get_fresh_l3_var l3_e =
  let my_compile_l3_v =
    compile_l3_v_generator
      l3_label_prefixer
      (fun v ->
  ...
  | L3If (test_v, thn_e, els_e) ->
    ...
    and then_insts = compile_l3_e l3_label_prefixer l3_var_map get_fresh_l3_var thn_e
    and else_insts = compile_l3_e l3_label_prefixer l3_var_map get_fresh_l3_var els_e
    in
  ...
  | L3D inst_d ->
    ...
    compile_l3_d my_compile_l3_v result_reg inst_d
  | _ -> failwith "l3c: invalid l3_e"


(* rename variables during compile function with prefix *)
(* f -> expr *)
let compile_l3_f l3_fun_label_prefixer l3_label_prefixer = function
  | L3Fun (fun_labl, vars, body_e) when (is_label fun_labl && List.for_all is_var vars) ->
    ...
    in
    let labl =
      compile_l3_v_generator l3_fun_label_prefixer rename_l3_args (L3Label fun_labl) (* fun_labl must be label check during parser *)
...
let compile_l3_p = function
  | L3Prog (entry_e, fundefs) ->
    let l3_label_prefixer labl =
      ":" ^ l3_label_prefix ^ String.sub labl 1 ((String.length labl) - 1)
    and identical i = i in
    let entry_fun = L3Fun (l3_entry, [], entry_e)
    and fs =
      List.map (compile_l3_f l3_label_prefixer l3_label_prefixer) fundefs
    in
    Expr (Atom l3_entry :: (compile_l3_f identical l3_label_prefixer entry_fun) :: fs)
